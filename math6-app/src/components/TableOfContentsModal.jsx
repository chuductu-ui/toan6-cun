                ...
                {(!chapter.lessons || chapter.lessons.length === 0) ? (
                  ...
                  <div className="toc-lessons-list">
                    {chapter.lessons.map((lesson) => {
                      const status = lessonStatusMap[lesson.id] || { isActive: false, isCompleted: false };
                      ...
                      const statusText = status.isCompleted ? 'Đã hoàn thành' : status.isActive ? 'Sẵn sàng học' : 'Đang khóa';

                      return (
                        <button
                          key={lesson.id}
                          className={`toc-lesson-item ${status.isCompleted ? 'completed' : ''} ${status.isActive ? 'active' : 'locked'}`}
